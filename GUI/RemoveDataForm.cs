...
{    
    public partial class RemoveDataForm : Form
    {
        ...
        //-------------------------------------------------------------------------------------------------------------------
        /**
         * Zadaniem metody jest usunięcie zaznaczonych danych
         */
        private int RemoveData()
        {
            int res = -1;
            int countOk = 0;
            Sesion sesionToRemove;
            for (int i = 0; i < listViewData.CheckedItems.Count; i++)
            {
                sesionToRemove = (Sesion)listViewData.CheckedItems[i].Tag;
                if (Factory.DataBase.RemoveData(sesionToRemove) == 0)
                    countOk++;
            }
            if (countOk == listViewData.CheckedItems.Count)
                res = 0;
            return res;
        }
        ...
        //-------------------------------------------------------------------------------------------------------------------
        /**
         * Obsluga zdarzenia przycisku Remove
         */
        private void btnOK_Click(object sender, EventArgs e)
        {
            if (listViewData.CheckedItems.Count > 0)
            {
                DialogResult result = MessageBox.Show("Are you sure you want to delete data ?", "Alert", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (result == DialogResult.Yes)
                {
                    int res =  RemoveData();
                    ShowAvalibleArchiveData();
                    if(res == 0)
                        MessageBox.Show("Data has been successfully deleted", "Alert", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
        //-------------------------------------------------------------------------------------------------------------------
